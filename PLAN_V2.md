...
22. ✅ **PR #30 — Ollama constrained decoding + DeepSeek json_object** SHIPPED 2026-04-27 18:32 UTC. **Verified over 7.6 days post-merge: Ollama parse rate 22.7% → 100% (129/129 real bot calls).** DeepSeek rescue dropped to 1 across the entire window (vs ~75% pre-fix). Sample post-fix output is substantive — model cites historical priors, company-specific context, market-bias deltas. Constrained decoding via `format=ANALYSIS_SCHEMA` makes non-JSON output mathematically impossible.
23. ❌ **PR #31 — Local model swap** CANCELLED. PR #30 alone was sufficient: `llama3.2:3b` with constrained decoding produces both parseable AND substantive content. Swap to `qwen2.5:7b-instruct` would add no value. `phi-4:14b` and `deepseek-r1:14b` remain available for offline backfill if/when fine-tune corpus building begins.

### Audit finding 2026-05-05 — gates over-restrictive

7.6 days post-PR-30, the bot has placed **zero new trades** despite AI signal now working. Of 1,798 recommendations across 182 trader runs:
- `solo_momentum_low_res` (PR #25): **1006 (56%)**
- `position_class_full`: **423 (24%)** — `long_or_uncertain` bucket stuck with 3 stale OPEN positions, cap is 2
- `negative_ev` (PR #22): **157 (9%)**
- `ai_veto`: 2
- Passed pre-flight: 210; **executed: 0** (the 210 die in `execute_trade` — likely Kelly returning 0 or live-EV recompute going negative)
